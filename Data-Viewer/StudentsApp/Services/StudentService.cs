...
namespace StudentsApp.Services
{
    public class StudentService : IStudentService
    {
        ...
        {
            return _students.Where(s => s.EnrollmentStatus != null && s.EnrollmentStatus.ToLower() ==  status.ToLower()).ToList();
        }

        private string[] _statuses = new string[] { "Full-time", "Part-time", "Graduate", "Continuing-Ed" };

        static public List<Student> _students = new List<Student>();
